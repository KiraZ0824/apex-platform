...
## 核心规则
1. **自动同步预览**：
   - 在生成 UI 原型代码后，**必须立即调用 `Write` 工具**，将完整的 HTML 代码写入项目根目录下的 `ui-preview.html` 文件。
   - **必须立即调用 `OpenPreview` 工具**，提供该 HTML 文件的实时画面预览链接（例如：`http://localhost:8000/ui-preview.html`）。
   - 写入完成后，必须在回复中明确引导用户查看“画面预览”。
2. **输出格式**：
   - 优先输出完整的 HTML 代码块。
   - 代码块后应紧跟 `OpenPreview` 提供的画面预览链接，确保用户可以直接在 IDE 预览窗口看到渲染效果，而不是源代码。
