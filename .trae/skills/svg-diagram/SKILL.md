...
1. **输出 ` ```html ` 代码块**，不要用 ` ```mermaid `。
2. **自动同步预览**：
   - 每次生成 SVG 后，必须立即调用 `Write` 工具，将完整的 HTML 代码（包含 SVG 和必要的预览样式）写入项目根目录下的 `diagram-preview.html` 文件。
   - 写入完成后，必须提示用户通过已有的预览链接查看结果。
3. **不要调用 generate_image 工具**。
4. **只输出 HTML 片段**（不含 DOCTYPE/html/head/body），格式：`<style>` → `<svg>`。
5. **先输出图表，再做文字解释**（如有必要）。
